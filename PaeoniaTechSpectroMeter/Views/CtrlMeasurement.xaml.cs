...
namespace PaeoniaTechSpectroMeter.Views
{
    public partial class CtrlMeasurement : UserControl
    {
            ...
            if ((string)BtnMeasurement.Content == "Start Measurement")
            {
                ...
                if (mmgr.ReadDetector.checkfeildentry())
                {
                    if (!mmgr.ReadDetector.checkfeildentryexist())
                    {
                        ...
                        mmgr.ReadDetector.MeasurementCompletedat = $" Please have a new combination of Sample Name and Analysis Type different from today's measurements before starting measurements.";
        ...
        {
            string serr = ""; //Testing
            serr= mmgr.ReadDetector.BrowseLocation(); //BrowseLocation
            if(serr!="")
